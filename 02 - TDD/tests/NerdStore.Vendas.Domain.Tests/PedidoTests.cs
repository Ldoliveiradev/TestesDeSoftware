...
namespace NerdStore.Vendas.Domain.Tests
{
    public class PedidoTests
    {
        ...
        [Trait("Categoria", "Vendas - Pedido")]
        public void AtualizarItemPedido_ItemÑaoExisteNaLista_DeveRetornarException()
        {
            // Arrange
            var pedido = Pedido.PedidoFactory.NovoPedidoRascunho(Guid.NewGuid());
            var pedidoIemAtualizado = new PedidoItem(Guid.NewGuid(), "Produto Teste", 5, 100);

            // Act & Assert
            Assert.Throws<DomainException>(() => pedido.AtualizarItem(pedidoIemAtualizado));
        }
        ...
        [Trait("Categoria", "Vendas - Pedido")]
        public void AtualizarItemPedido_ItemUnidadesAcimePermitido_DeveRetornarException()
        {
            // Arrange
            var pedido = Pedido.PedidoFactory.NovoPedidoRascunho(Guid.NewGuid());
            var produtoId = Guid.NewGuid();
        ...
        public void RemoverItemPedido_ItemExistente_DeveAtualizarValorTotal()
        {
            ...
            var pedidoItem2 = new PedidoItem(pedidoId, "Produto Teste", 3, 15);
